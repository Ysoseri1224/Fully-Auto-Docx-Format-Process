...
### Bug 修复
- 修复 Pandoc 调用参数粘连问题：`--reference-doc=路径` 改为独立参数，正确处理包含空格和中文的路径

## 0.7.0

### 文稿批注（全新功能）
- 新增顶层视图「文稿批注」，交互式 OOXML 批注插入/编辑/删除
- Run 级别文本渲染，精确字符偏移选区锚定
- 批注按文稿位置排序显示
- 批注编辑：修改已有批注内容，直接写回 DOCX
- 批量删除：勾选多条批注一键删除
- Ref 标记自动转化：加载文件时检测 `<!-- start:ref:xxx -->...<!-- ref:xxx -->` 标记对，弹出选择对话框，一键转化为批注占位
- 批注直接写入源文件，UI 中明确提示用户
